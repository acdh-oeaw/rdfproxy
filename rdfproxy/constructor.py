...
class _QueryConstructor:
    ...
    def __init__(
        ...
        self.orderby_bindings_map = OrderByFieldBindingsMap(model)

        self.group_by: str | None = self.bindings_map.get(
            model.model_config.get("group_by")
        )
        self.order_by: str | None = self.orderby_bindings_map.get(
            self.query_parameters.order_by
        )
